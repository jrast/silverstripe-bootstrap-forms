...
<% else %>
	<div class="ss-uploadfield-item ss-uploadfield-addfile<% if $Items && $displayInput %> borderTop<% end_if %>" <% if not $displayInput %>style="display: none;"<% end_if %>>
		...
		<div class="ss-uploadfield-item-info">
			...
			<label class="ss-uploadfield-fromcomputer" title="<% _t('UploadField.FROMCOMPUTERINFO', 'Upload from your computer') %>">
			...
			<% if not $autoUpload %>
				<button class="ss-uploadfield-startall" title="<% _t('UploadField.STARTALLINFO', 'Start all uploads') %>"><% _t('UploadField.STARTALL', 'Start all') %></button>
			<% end_if %>
			<div class="clear"><!-- --></div>
		</div>
		<div class="clear"><!-- --></div>
